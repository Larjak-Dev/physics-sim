...
// Placeholder for platform specific URL opening
#ifndef PlatformOpenURLInBrowser
inline void PlatformOpenURLInBrowser(const char *url)
{
    (void)url;
}
#endif
...
class ColSwitchTemp
{
  ...
  public:
    ColSwitchTemp(ImGuiCol toChange_, ImVec4 to_) : toChange(toChange_)
    {
...
inline void SetupImGuiStyle(bool bStyleDark_, float alpha_)
{
    ...
    if (bStyleDark_)
    {
        ...
        // Classic ImGui Blue for Inputs
        style.Colors[ImGuiCol_FrameBg] = ImVec4(0.26f, 0.59f, 0.98f, 0.35f);
        style.Colors[ImGuiCol_FrameBgHovered] = ImVec4(0.26f, 0.59f, 0.98f, 0.50f);
        style.Colors[ImGuiCol_FrameBgActive] = ImVec4(0.26f, 0.59f, 0.98f, 0.67f);
    }
    else
    {
        ...
        // Classic ImGui Blue (Lighter) for Inputs
        style.Colors[ImGuiCol_FrameBg] = ImVec4(0.26f, 0.59f, 0.98f, 0.20f);
    }
}
...
inline bool MenuItemCheckbox(const char *name_, bool *pB_)
{
    ...
    if (*pB_)
    {
        snprintf(tempBuffer, sizeof(tempBuffer), "%s %s", ICON_FA_SQUARE_CHECK, name_);
    }
    else
    {
        snprintf(tempBuffer, sizeof(tempBuffer), "%s %s", ICON_FA_SQUARE, name_);
    }
...
inline bool RadioSelectable(const char *label, int *v, int v_button, ImGuiSelectableFlags flags = 0,
...
struct FrameTimeHistogram
{
    ...
    void Update(float deltaT_)
    {
        ...
        float hitch = std::abs(lastdT - deltaT_);
    ...
    void PlotRefreshLines(float total_ = 0.0f, float *pValues_ = NULL)
    {
        ...
        if (pValues_)
        {
            ...
            for (int i = 0; i < NUM && mark < NUM_MARKERS; ++i)
            {
                ...
                {
                    float xP = std::ceil((float)(i + 1) / (float)NUM * (max.x - min.x) +
    ...
    {
        ImVec2 wRegion = ImGui::GetContentRegionAvail();
...
// --- PROPERTY GRID ADDITIONS (MODERN IMGUI) ---
inline bool BeginPropertyGrid(const char *label, float labelWidth = 80.0f)
{
...
inline bool PropertyRow(const char *label, const char *id, double *value, const char *format = "%.2e",
                        ImGuiInputTextFlags flags = 0)
{
    ...
    return ImGui::InputDouble(id, value, 0.0, 0.0, format, flags);
}

inline void PropertyLabel(const char *label, const char *value)
{
    ImGui::TableNextRow();
    ImGui::TableNextColumn();
    ImGui::AlignTextToFramePadding();
