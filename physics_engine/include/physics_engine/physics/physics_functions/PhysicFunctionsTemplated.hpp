...
template <double g> inline vec3d FreeFallForceFunc(vec3d pos, const Body &self, const EnvironmentBase &env)
{
...
template <auto AccelFunc> Body implicitEulerBodyStep(const Body &body, double delta_time, const EnvironmentBase &env)
{
    if (body.is_locked)
        return body;

    const auto a_0 = AccelFunc(body.pos, body, env);
...
EnvironmentBase implicitEulerStepFunc(const EnvironmentBase &env, double delta_time, StepBuffer &buffer)
{
    ...
    {
        env_new.bodies.emplace_back(implicitEulerBodyStep<AccelFunc>(body, delta_time, env));
    }
...
template <auto AccelFunc> Body verletBodyStep(const Body &body, double delta_time, const EnvironmentBase &env)
{
    if (body.is_locked)
        return body;
...
EnvironmentBase verletStepFunc(const EnvironmentBase &env, double delta_time, StepBuffer &buffer)
{
    ...
    for (const Body &body : env.bodies)
    {
        env_new.bodies.emplace_back(verletBodyStep<AccelFunc>(body, delta_time, env));
    }
...
inline EnvironmentBase advance(const EnvironmentBase &start, const std::vector<Derivates> &derivates, double delta_time)
{
    auto env_new = start;
...
template <auto AccelFunc> void calcDerivates(const EnvironmentBase &env, std::vector<Derivates> &bufferData)
{
    for (auto &&[i, body] : std::views::enumerate(env.bodies))
    {
        ...
        else
        {
            ...
            bufferData[i].k = AccelFunc(body.pos, body, env);
        }
    }
}

inline EnvironmentBase advanceWithSum(const EnvironmentBase &start, const std::vector<Derivates> &der_0,
...
template <auto AccelFunc> EnvironmentBase RK4StepFunc(const EnvironmentBase &env, double delta_time, StepBuffer &buffer)
{
    ...
    calcDerivates<AccelFunc>(env, derivates_1);

    const auto env_mid2 = advance(env, derivates_1, delta_time / 2.0);
    calcDerivates<AccelFunc>(env_mid2, derivates_2);

    const auto env_mid3 = advance(env, derivates_2, delta_time / 2.0);
    calcDerivates<AccelFunc>(env_mid3, derivates_3);

    const auto env_mid4 = advance(env, derivates_3, delta_time);
    calcDerivates<AccelFunc>(env_mid4, derivates_4);
